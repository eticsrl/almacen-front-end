<template>
    <div>
      <el-card>
        ...
        <div class="header">
          ...
          <div class="actions">
            <el-button type="primary" :icon="Search" :loading="report.loading" @click="fetchData">Consultar</el-button>
            <el-button type="info" :icon="Refresh" :loading="report.loading" @click="resetFilters">Limpiar</el-button>
            <el-button type="success" :icon="Printer" :disabled="!report.hasData" @click="openPrint">Imprimir</el-button>
          <!-- dentro de <div class="actions"
            <el-dropdown>
              <el-button :icon="Download" :disabled="!report.hasData">
                Exportar
              </el-button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click="exportCsv">CSV</el-dropdown-item>
                  <el-dropdown-item @click="exportXlsx">Excel (.xlsx)</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>-->
        ...
        <div class="filtros-contenedor">
          ...
          <!-- Filtro: Tipo de documento (cat=1) -->
          <el-select
        ...
        >
          <template v-if="group === 'detalle'">
            ...
            <el-table-column prop="numero" label="Nº Ingreso" width="100" />
            <el-table-column prop="tipo_documento" label="Tipo Ingreso" width="180" show-overflow-tooltip />
            <el-table-column prop="proveedor" label="Proveedor" width="220" show-overflow-tooltip />
          ...
          <template v-else-if="group === 'proveedor'">
        ...
        <!-- Totales -->
        <div class="totales">
          <div class="totales-item"><span class="lbl">Total Cantidad</span><span class="val">{{ n(report.totalCantidad) }}</span></div>
          <div class="totales-item"><span class="lbl">Total Valor</span><span class="val">{{ money(report.totalValor) }}</span></div>
        </div>
      </el-card>
  
      <!-- Modal de impresión -->
      <el-dialog 
          v-model="showPrint" :title="`Vista previa - Ingresos por fecha (${groupLabel})`" width="1100px" :close-on-click-modal="false">
        <div ref="printArea" class="doc">
          <div class="doc-header">
            <div class="logo"><img :src="logoUrl" alt="Logo" /></div>
            <div class="center">
              <h1>SEGURO SOCIAL UNIVERSITARIO POTOSÍ</h1>
              <h2>Ingresos por fecha — {{ groupLabel }}</h2>
          ...
          <table class="tabla">
            ...
            <thead>
              <tr v-if="group === 'detalle'">
                <th>Fecha</th><th>Nº Ingreso</th><th>Tipo Ingreso</th><th>Proveedor</th><th>LINAME</th><th>Medicamento</th><th>Presentación</th><th>Lote</th><th>Venc.</th><th class="num">Cant.</th><th class="num">Costo Unit.</th><th class="num">Costo Total</th>
              </tr>
              ...
              <tr v-else-if="group === 'proveedor'">
                <th>Proveedor</th><th class="num">Cantidad</th><th class="num">Valor</th>
              </tr>
  ...
  import { useEntryReportStore } from '@/stores/entryReportStore'
  import { useUserStore } from '@/stores/userStore'
  import { useDocumentTypeStore } from '@/stores/documentTypeStore' // para cat=1
  import { Printer, Refresh, Search, Download } from '@element-plus/icons-vue'
 
  import logoSSU  from '@/assets/imagenes/logo-ssu.png'
  import logoSSUE from '@/assets/imagenes/logo ssue.png'
   
  const report = useEntryReportStore()
  ...
  onMounted(async () => {
    // Tipos de documento de ingreso (cat=1)
  ...
  async function fetchData () {
    ...
    else if (g === 'proveedor') await report.fetchPorProveedor()
  ...
  function resetFilters () {
    ...
    report.setTiposDocumento([])
    if (entityId.value) report.setEntity(entityId.value)
    fetchData()
  }
  
  // helpers
  function fmt (v, f = 'DD/MM/YYYY') { return v && dayjs(v).isValid() ? dayjs(v).format(f) : '-' }
  function n (x) { const v = Number(x || 0); return v.toLocaleString('es-BO', { maximumFractionDigits: 0 }) }
  function money (x) { const v = Number(x || 0); return v.toLocaleString('es-BO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }
  ...
  const printColspan = computed(() => {
    ...
    if (group.value === 'proveedor') return 3
    ...
    const docHtml = `
      <html>
        <head>
          <meta charset="utf-8" />
          <base href="${baseHref}">
          <title>Ingresos por fecha — ${groupLabel.value}</title>
          <style>${css}</style>
        </head>
        <body>${htmlBody}</body>
      </html>
  ...
  .totales { display:flex; gap:18px; justify-content:flex-end; margin-top:8px; }
  .totales-item .lbl { color:#666; margin-right:8px; }
  .totales-item .val { font-weight:700; }
  
  /* Solo preview (no impresión) */
