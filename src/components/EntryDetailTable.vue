<template>
  <div>
    <el-button v-if="!props.readonly" type="primary" icon="el-icon-plus" @click="addRow" class="mb-2">
      Agregar Item
    </el-button>

    <el-table :data="details" border style="width: 100%" :row-class-name="rowClassName" show-summary
      :summary-method="getSummaries">
      <!-- Nombre -->
      <el-table-column label="Medicamento" prop="medicamento" width="330">
        <template #default="{ row }">
          <el-select 
            ...
            <el-option 
              ...
              :key="item.id" 
              pue :label="item.nombre_generico" 
              :value="item.id"
      ...
      <el-table-column 
        label="Presentación" 
        prop="formafarmaceutica" 
        width="150">
          <template #default="{ row }">
            <el-input 
              v-model="row.formafarmaceutica" 
              size="small" 
              readonly 
              :disabled="props.readonly" />
          </template>
      ...
      <!-- Fecha vencimiento -->
      <el-table-column label="Fecha Vencimiento" prop="fecha_vencimiento" width="120">
        <template #default="{ row }">
          <el-date-picker v-model="row.fecha_vencimiento" :disabled="props.readonly" size="small" type="date"
            placeholder="Elija una fecha" format="DD/MM/YYYY" value-format="YYYY-MM-DD" />
        </template>
      </el-table-column>

      <!-- Cantidad -->
      <el-table-column label="Cant." prop="cantidad" width="80">
        <template #default="{ row }">
          <el-input-number v-model="row.cantidad" :disabled="props.readonly" :min="1" size="small"
            controls-position="right" @change="recalculate(row)" />
        </template>
      </el-table-column>

      <!-- Precio unitario -->
      <el-table-column label="P. Uni." prop="costo_unitario" width="80">
        <template #default="{ row }">
          <el-input-number 
            v-model="row.costo_unitario" 
            :disabled="props.readonly" 
            :min="0" 
            size="small"
            ...
            :step="0.01"
            :precision="2"
             @change="onCostoUnitarioChange(row)" />
        </template>
      </el-table-column>

      <!-- Precio total -->
      <el-table-column label="Subtotal" prop="costo_total" width="120">
        <template #default="{ row }">
          <el-input-number 
            v-model="row.costo_total" 
            :disabled="props.readonly" 
            :min="0" 
            size="small"
            controls-position="right" 
      ...
      <!-- Acciones -->
      <el-table-column label="Acción" width="80" v-if="!props.readonly">
        <template #default="{ $index, row }">
          <el-button :icon="Delete" type="danger" size="small" @click="removeRow($index, row)" />
        </template>
      </el-table-column>
      <el-table-column label="Estado" prop="estado" width="120">
        <template #default="{ row }">
          <el-tag :type="getEstadoTagType(row.estado_id) || undefined" effect="dark" disable-transitions>
            {{ row.estado }}
          </el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="selectDialogVisible" title="Seleccionar ítem para reingreso" border style="width: 75%">
      <el-input v-model="search" placeholder="Buscar por nombre" clearable style="margin-bottom: 10px; width: 300px" />
...
const updateMedicine = (row, medicineId) => {
  ...
  if (selected) {
    ...
    //row.medicamento = selected.liname
    row.liname = selected.liname
    console.log("medicamento seleccionado:", selected.liname)
    row.formafarmaceutica = selected.formafarmaceutica
    console.log("forma farmacéutica:", selected.formafarmaceutica)
  }
}
...
const recalculate = (row) => {
  // Asegura 2 decimales en costo_unitario y recalcula el total con 2 decimales
  if (row.costo_unitario != null) {
    row.costo_unitario = Number(parseFloat(row.costo_unitario || 0).toFixed(2))
  }
  if (row.cantidad > 0 && row.costo_unitario > 0) {
    row.costo_total = parseFloat((row.cantidad * row.costo_unitario).toFixed(2))
  } else {
...
const onCostoUnitarioChange = (row) => {
  // Formatea el unitario con 2 decimales y recalcula subtotal
  if (row.costo_unitario != null) {
    row.costo_unitario = Number(parseFloat(row.costo_unitario || 0).toFixed(2))
  }
  recalculate(row)
...
const recalculateFromTotal = (row) => {
  if (row.cantidad > 0 && row.costo_total > 0) {
    row.costo_unitario = Number(parseFloat(row.costo_total / row.cantidad).toFixed(2))
  } else {
...
const getSummaries = ({ columns, data }) => {
  // Crear un array para almacenar los subtotales, debe tener el mismo número de elementos que las columnas
  const sums = []

  // 1. Calcular el subtotal de costo_total
  const totalCost = data.reduce((total, row) => {
    // Usar parseFloat para asegurar que la suma sea numérica
    const totalForRow = parseFloat(row.costo_total || 0)
    return total + totalForRow
  }, 0)

  // 2. Llenar el array de sumas.
  // La columna 'Precio Total' (prop: 'costo_total') es el índice 7 (contando desde 0, o 8 si se cuenta la primera)

  columns.forEach((column, index) => {
    if (index === 0) {
      // Primera columna: Mostrar la etiqueta "Subtotal"
      sums[index] = 'Total ingreso'
    } else if (column.property === 'costo_total') {
      // Columna 'Precio Total': Mostrar la suma formateada
      // Aquí se usa Intl.NumberFormat para formatear como moneda (ej: 1,234.56)
      sums[index] = totalCost.toFixed(2) + ' '

    } else {
      // Resto de columnas: Dejar vacío
      sums[index] = ''
    }
  })
