  ...
  <div>
    <el-card class="box-card">
      ...
      <ServicePersonalDetailTable 
        :data="servicePersonalStore.servicePersonals"
        @edit="openModal"
        @delete="handleDelete"
      />
    </el-card>
...
const servicePersonalStore = useServicePersonalStore()
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingServicePersonal.value) {
      await servicePersonalStore.updateServicePersonal(editingServicePersonal.value, form)
      ElMessage.success('Servicio del personal actualizado')
    } else {
      await servicePersonalStore.createServicePersonal(form)
...
const handleDelete = (id) => {
  ElMessageBox.confirm('¿Estás seguro de eliminar este servicio del personal?', 'Confirmar', {
    ...
    try {
      await servicePersonalStore.deleteServicePersonal(id)
...
onMounted(() => {
  servicePersonalStore.fetchServicePersonals()
})
</script>
